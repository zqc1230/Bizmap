...
function gmbc(type, th) {
    //type:0-main 1-sub
    if (editSwitch == "") {
        $(".row0Div,.listSub").css('background-color', 'transparent');
        if (type == 0) {
...
function click_search(kw, sl, notes, gpsll, st) {//st: search type[]
    ...
    if (gps.length < 1) {
        ...
        $("#list").html("<center> <br><br><br>Positioning ...</center>");
        // $("#list").html(" Positioning...<br><br>");
        // setTimeout("clickSearch('" + kw + "','" + sl + "','" + notes + "','" + gpsll + "','" + st + "')", 2000);
        setTimeout("clickSearch('" + kw + "','" + sl + "','" + notes + "','" + gpsll + "','" + st + "')", 3000);
    }
    else {
        clickSearch(kw, sl, notes, gpsll, st);
    }
...
function clickSearch(kw, sl, notes, gpsll, st) {
    if (!firser) {
        try { sia.abort(); }
        catch (e) { jsErrorMSG += "<br />line:" + e.line + "-message:" + e.message; }
    }
    seachtime++;
        ...
        sia = $.ajax({
            ...
            success: function (a) {
                ...
                if (a.toLowerCase().indexOf("sorry") != -1) {
                    $("#list").height(188); //95
                    $("#container").height(185); //95
                    //$("#listBottom").height(110);
                    $("#list-top").hide();
                }
                else {
                    $("#container").height(205); //210
                    //(#10#)
                    //alert(a);
                    try {
                ...
                if ($("#map_canvas").height() > 300) {//半屏    
                    $("#list").css({ 'z-index': '333', 'height': '90px' });
                    $("#list table").css({ "background-color": "silver", "opacity": "0.8" });
                    $("#container").css({ "height": "92px", "margin-top": "290px" });
                    $("#listBottom1").height(113);
            ...
            error: function (a) {
                ...
                var resu;
                if (kw == "dic$") {
                    resu = "<p id='stcs_result1'>&nbsp;Sorry, no location found <br/><br></p><div><div id=\"list-mp\">Main Page</div><div id=\"list-re\">Write something to us.</div></div>";
                }
                else {
                    resu = "<p id='stcs_result1'>&nbsp;Sorry, no location found for <br/>&nbsp;'" + kw + "' <br></p><div><div id=\"list-mp\">Main Page</div><div id=\"list-re\">Write something to us.</div></div>";
                }
                $("#list").html(resu);
            }
        });
...
$(document).ready(function () {
    ...
    $("#list-re").live('click', function () {
        if ($("#map_canvas").height() > 300) {
            //alert(1749);
            document.getElementById('Fullscreen').style.WebkitTransform = "rotate(180deg)";
            $("#map_canvas").animate({ height: "210px" }, 500);
            setTimeout("google.maps.event.trigger(map, 'resize')", 600);
            $("#tools").animate({ marginTop: "0px" });
            if (keywords != "none") {
                $("#list").css({ 'z-index': '0', 'height': '205px', "border": "0px" });
                $("#list table").css({ "background-color": "transparent", "opacity": "1" });
                $("#listBottom1").height(0);
                $("#container").css({ "height": "205px", "margin-top": "211px" });
                $('#list').touchScroll('update');
            }
            inputSwitch = true;
            setTimeout('newReport(2)', 1000);
        }
        else {
            inputSwitch = true;
            newReport(2);
...
function newReport(x) {
    ...
    trf += '<span id="CloseReport" onclick="CloseReport(\'' + x + '\')"><span class="chahao">×</span></span> <span id="Report" onclick="Next(\'' + x + '\')">Next</span><div style="height:10px;clear: both;"></div></div>';
    $("#rf").html(trf);
    google.maps.event.addDomListener(document.getElementById("ReportsForm"), 'touchstart', function (e) {
        $("#tools").height(0);
        $("#tool,#tools").hide();
    ...
    if ($("#map_canvas").height() > 300) {//半屏
        $("#rf").css({ 'z-index': '333', 'opacity': '0.8' });
    }
}
var reportinfo;
var newbizgps = "";
...
function Next(x) {
    ...
    if ($("#Report").html() == "Next") {
        ...
        if (reportinfo != "") {
            $("#Report, #CloseReport").css("margin-top", "0px");
            //#region
            $("#tools").height(45);
            $("#tool,#tools").show();
            opentools = true;
...
function click_map(x) {
    ...
    if (editSwitch == "") {
        ...
        if (keywords == "none") {
            $("#list").html("<center><br><br><br><br><div id=\"backonList\" onclick=\"click_back()\">Main page</div><div onclick=\"dis(event);newReport(2);\" id=\"list-re\">Write something to us.</div></center>");
    ...
    setTimeout('monitor.tomap()', 1000);
}
function ToMap(x) {
    $("#saved").hide();
    $("#container").show();
    $("#menu,#function,#saved,#nb,#account,#main,#tb_search,#divBM,#menuLists,#editDone,#bookMarks").hide();
...
function click_back() {
    ...
    if ($("#map_canvas").height() < 300) {//半屏
        ...
        $("#map").css('-webkit-transform', 'matrix(0.082, 0, 0, 0.13, 0, 0)');
        //setTimeout('$("#map").animate({ top: "-75px" },500)', 500);
        // $("#map").animate({ left: "132px" });
        $("#map").animate({ left: "132px", top: "-76px" }, 500);
    }
    else {//full screen
        $("#map").css('-webkit-transform', 'matrix(0.082, 0, 0, 0.065, 0, 0)');
        $("#map").animate({ left: "132px", top: "-82px" }, 500);
    }
...
LatLngControl.prototype.updatePosition = function () {
    ...
    if (wid && hei) {
        var point = new google.maps.Point(wid, hei);
    }
    else {
        var point = new google.maps.Point(160, 105);
    }
...
function initialize(param) {
    ...
    //document.getElementById("centerControlDiv").style.top = '103px';
    ...
    google.maps.event.addDomListener(document.getElementById('map'), 'touchend', function (e) {
        //setTimeout('if (!hisw && tomenu) {click_map(1);}', 300);
        if (!hisw && tomenu) {
            click_map(1);
        }
...
function cross() {
    ...
    if ($("#cross").is(":visible")) {
        ...
        $("#cross").html('<br><br><br><br><div id="c"><span class="tip">Use center as</span><div class="cross" onclick="c1()" id="c1"> Current Location</div><span class="tip1">or</span><div class="crossit"  onclick="c3()" id="c3">Save it</div><div class="crossit" onclick="c2()" id="c2">Share it</div></div><div onclick="closecross()" id="closecross"><span class="chahao">×</span></div>');
        $("#cross").show();
...
//#endregion
//#region  全屏Full screen
function BottomControl(bottomDiv, map) {
    bottomDiv.style.paddingTop = '0px';
    var tUI = document.createElement('div');
    tUI.style.width = '37px';
    tUI.style.height = '25px';
    tUI.style.opacity = '0.8';
    tUI.style.border = '0px';
    tUI.style.background = 'url(images/up_green30.png)';
    tUI.style.WebkitTransform = "rotate(180deg)";
    tUI.style.marginTop = '-33px';
    tUI.style.zIndex = '300';
    tUI.id = 'Fullscreen';
    ...
    google.maps.event.addDomListener(tUI, 'click', function () {
        if (hisw && !isSearching) {
            if ($("#map_canvas").height() < 300) {
                tUI.style.WebkitTransform = "rotate(0deg)";
                $("#map_canvas").animate({ height: "417px" }, 500);
                setTimeout("google.maps.event.trigger(map, 'resize')", 600);
                $("#tools").animate({ marginTop: "80px" });
                if (keywords != "none") {
                    $("#list").css({ 'z-index': '333', 'height': '90px' });
                    $("#list table").css({ "background-color": "silver", "opacity": "0.8" });
                    $("#container").css({ "height": "92px", "margin-top": "290px" });
...
function watchgps(x) {
    ...
    onError,
     { enableHighAccuracy: true, timeout: 6000 }
        ...
        //gpschange = '';
        watchID = watchId;
        setTimeout("stopwatchGPS('" + watchId + "','" + x + "')", 6500);
    ...
    if (hisw) {
        ...
        try {
            ...
            if (oldGps && newGPS && ("" + oldGps != "" + newGPS)) {
                ...
                if (seachpara[1] == 'Nearby' || seachtime < 2) {//nearby、第一次搜索
...
var curb = 0; //当前倍率
var curt = true; //当前方向
function changeBorder(id, min, max) {
    if (curb >= min && curb < max && curt) {
        curb++;
        //$("#" + id).height(curb);
        //$("#" + id).css('-moz-border-radius', '"' + curb + 'px"');
    }
    else {
        curb--;
        curt = false;
    ...
    document.getElementById(id).style.borderRadius = curb + 'px';
            ...
            if (!isNaN(i)) {//序号
                ...
                if (exi == i) {//如果是同一个，则放大
                    ...
                    try {
                        ...
                        var ico = document.getElementById("list").childNodes[2 * exii + 1].childNodes[0].childNodes[0].childNodes[1].childNodes[0];
...
function centerwithGPS(plat, plng, k, a, slat, slng, elat, elng, isk, exkk, isc, from) {
    ...
    if ((exk == k) && (!isk) && (from != '1')) {//是前一个，且不要对k进行处理
        ...
        */
        ...
        var gpslanlng0 = new google.maps.LatLng(gpsLatLng[0], gpsLatLng[1]);
        if (x2 - x1 > 0) {
            var sw_x = x1;
            var ne_x = x2;
        }
...
function click_details(a, b, c, d, e, f, g, h, i, j, k, plat, plng, ccode, th, exii, dis, scode) {
    ...
    //#region 如果是普通搜索
        ...
        if (getAccountLevel() >= 0) {
            fun = '<div id="p3_ownernotes"><div class="cross4" onclick="shareBiz(true,\'' + a + '\',\'' + tb + '\',\'' + c + '\',\'' + d + '\',\'' + ccode + '\')" id="shareBiz">Share</div><div class="cross5" onclick="saveSearch(\'' + a + '\',\'' + tb + '\',\'' + c + '\',\'' + d + '\',\'' + ccode + '\')" id="saveBiz">Save</div><div class="cross6" onclick="newReport(\'' + ccode + '\')">Report</div><div id="closeDetails" onclick="closeDetails();"><span class="chahao">×</span></div></div>';
        }
        else {
    ...
    if ($("#map_canvas").height() > 300) {//半屏
        $("#page3_details").css({ 'z-index': '333', 'opacity': '0.8' });
    }
}
//#endregion
...
var exnum;
function cc(th, f) { //changecolor(this,from)  from  [0:列表  1:详细页序号按钮  2:ATM  3:距离]
    if (exnum) {
        exnum.style.backgroundImage = 'url("./images/icon_L.png")';
    }
    try {
...
Monitor.prototype.tomap = function () {
    try {
        //debugger;
        if (hisw) {
            if ((!($("#map").is(":visible") && $("#map").css("-webkit-transform").indexOf("scale(1)") != -1)) && monitorTomapTime < 5) {
                $("#map").show();
                $("#map").css('-webkit-transform', 'scale(1)');
                monitorTomap = setTimeout('monitor.tomap()', 1000);
                monitorTomapTime++;
            }
            else {
