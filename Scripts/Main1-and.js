...
//GPS状态[0:机器获取，1:手动定位（cross），2:自动定位（center）]
var gpsState = 2;
...
function search1() {
    ...
    if (editSwitch == "" && $.trim(kw2).length > 1) {//    if ($.trim(kw).length > 1) {
...
function defaultArrlist1() {
    ...
    //arrList = new Array();
    //如果有数据，则从
    if ((storage) && (storage.getItem("arrList")) && storage.getItem("coreVersion") == 4) {
        arrList = storage.getItem("arrList").split(";");
    }
    else {
        var i = 0;
        ...
        arrList[i++] = "Bank,Scotia Bank,BMO Bank,Royal Bank,TD Bank,HSBC Bank";
