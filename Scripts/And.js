...
$("#cpright").html(' Copyright ? 2012, Bizmap.ca, Beta 1.102');
...
function listMenu(arrList) {
    ...
    //$('#menu1').touchScroll();
    $("#menu,#menu1").height(parseInt(window.screen.height - 100));
...
function saveSearch(uaid, name, addr, uph, ccode, isshow, sos, scode) {
    ...
    */
    ...
    if (!isshow) {
        ...
        //name = ReplaceF(name);
        $("#page3_detail").html('<div id="Lsd"><br>Location saved.</div><div id="shareit" onclick="shareBiz(true, \'' + uaid + '\', \'' + name + '\',\'' + addr + '\', \'' + uph + '\', \'' + ccode + '\')">Share it</div><br><div id="backonList" onclick="click_back()">Go Back</div><div id="closeDetails" class="close"  onclick="closeDetails();"><span class="chahao">×</span></div>'); //<div id="closeDetails1" onclick="closeDetails();"><span class="chahao">× </span></div>
        $("#page3_details").show();
        $("#closeDetails").css("margin", "9px 25px");
    } else {
        if (sos) {//true:share
            $("#page3_detail").html('<div id="Lsd"><br>Location shared.</div><br><br><div id="closeDetails1"  class="close" onclick="closeDetails();"><span class="chahao">×</span></div><div id="backonList" onclick="click_back()">Go Back</div><div id="closeDetails"  class="close" onclick="closeDetails();"><span class="chahao">×</span></div>');
        }
        else {//false: saved
            $("#page3_detail").html('<div id="Lsd"><br>Location saved.</div><br><br><div id="closeDetails1"  class="close" onclick="closeDetails();"><span class="chahao">×</span></div><div id="backonList" onclick="click_back()">Go Back</div><div id="closeDetails"  class="close" onclick="closeDetails();"><span class="chahao">×</span></div>');
...
function click_search(kw, sl, notes, gpsll, st) {//st: search type[]
    ...
    if (gps.length < 1) {
        ...
        sivar[0] = setInterval('$("#list").html("<center> <br><br><br><br><br>Positioning ...</center>");', 100);
...
function clickSearch(kw, sl, notes, gpsll, st) {
    ...
    try {
        ...
        $("#list").html("<center> <br><br><br><br><br>Searching ...</center>");
        clearTimeout(stsia);
    } catch (e) {
        catche(e);
    }
        ...
        sia = $.ajax({
            ...
            success: function (a) {
                ...
                if (a.toLowerCase().indexOf("sorry") != -1) {
                    $("#list").height(250); //95  205
                    $("#container").height(250); //95  185 205
                    //$("#listBottom").height(110);
                    $("#list-top").hide();
                }
                else {
                    $("#container").height(270); //210 205
                    //(#10#)
                    //alert(a);
                    try {
...
$(document).ready(function () {
    ...
    $("#cpright").live('click', function () {
        ...
        $("#copyrightCanvas,#copyrightText").height(parseInt(window.screen.height - 145));
...
function a4() {
    if ($("#a41").is(":visible")) {
        ...
        $("#a41").hide();
        $("#closeDetailsaccount,#closeDetailsaccount1").css({ marginTop: "37px" });
        clearTimeout(tif);
        notif = true;
    }
    else {
...
function backToaccount(x) {
    ...
    try {
        if (who.indexOf('@') != -1) {
            ...
            $("#account").html(tacc1);
            $("#account").show();
            $("#closeDetailsaccount,#closeDetailsaccount1").css({ marginTop: "42px" });
            $("#s1Info, #s2Info, #s3Info").css('margin', '15px');
            if (closeLR == "L") {
                $("#closeDetailsaccount").html('');
                $("#closeDetailsaccount").css({ 'background': 'none', 'backgroundColor': 'transparent', 'box-shadow': 'none', 'border': '2px dotted white' });
...
function showsaved() {
    ...
    if (closeLR == "L") {
        $("#closesaved").html('');
        $("#closesaved").css({ 'background': 'none', 'backgroundColor': 'transparent', 'box-shadow': 'none' });
    }
    else {
        $("#closesaved1").html('');
        $("#closesaved1").css({ 'background': 'none', 'backgroundColor': 'transparent', 'box-shadow': 'none', 'border': '2px dotted white' });
...
function click_map2(x) {
    ...
    if (!hisw && !mapEve) {
        ...
        */
        ...
        //#endregion  
        // sttesthisw = setTimeout('toTop();testhisw();', 1000);
        //setTimeout('monitor.tomap(true)', 1000);    
        //debugger;
        if (closeLR == "R") {
...
function click_map(x) {
    ...
    if (!hisw && !mapEve) {
        ...
        setTimeout('toTop();', 1000);
        //  alert($("body").width());
        // setTimeout('alert($("body").width())', 1000);  //setTimeout('monitor.tomap(true)', 1000);
    }
}
var sttesthisw;
function testhisw() {
...
function click_back1() {
    ...
    try {
        ...
        infoinmappage = $('#list').html();
        //sia.abort();
        $("#map,#map_canvas").height(285);
        $("#tools,#tool").css({ top: '285px' });
        google.maps.event.trigger(map, 'resize');
...
function click_back() {
    ...
    */
    ...
    tomenu = true;
    // alert($("body").width());
    // setTimeout('alert($("body").width())', 1000);
    //setTimeout('monitor.menu()', 1000);
    //setTimeout('monitor.tomap(false);', 300);
    //mapEve = false;
}
...
function a5() {
    try {
        ...
        */
        ...
        pointdiv += '</div>';
        //==
        pointdiv += '<br/><span id="monthtspan" class="points">This month tickets</span><br/><div id="monthtdiv"><div id="monthtdiv1"><br><br><center>loading ……</center></div></div>';
        ...
        $.ajax({
            ...
            success: function (a) {
                try {
                    ...
                    //alert($("#pimg1").height());
                    if (a) {
                        //iseie,当月积分，总积分，Finding，Sharing，Reporting，Inviting，Bonus，彩票数目，彩票内容
                        ...
                        //如果用户积分1000以上，则向左移一点
                        if (a[1] >= 1000) {
                            $("#monthpdiv").css('margin-left', '30px');
                        }
                        if (a[2] >= 1000) {
                        ...
                        if (verStr.indexOf('iPhone') == -1 && verStr.indexOf('Android') == -1) {
                            $("#pimg1, #pimg2, #pimg3, #pimg4, #pimg5").height(15);
                            $("#tickets li").css('margin-left', '17px');
                        }
                        //alert($("#tickets li").css('margin-left'));
                    }
                } catch (e) { catche(e); }
            },
            error: function (a) {
...
function click_details(a, b, c, d, e, f, g, h, i, j, k, plat, plng, ccode, th, exii, dis, scode, biz_hours) {
    ...
    //onclick="cc('+this+');centerwithGPS('43.6762320','-79.3582850','2')"
...
function closep1() {
    $("#page3_details").css("background-color", "#d7d8d9");
    $("#page3_detail").html('<div id="Lsd"><br>Shared location saved.</div><br><br><br><br><div id="closeDetails1" class="close" onclick="closeDetails();"><span class="chahao">×</span></div><div id="backonList" onclick="click_back()">Go Back</div><div id="closeDetails2" class="close" onclick="closeDetails();"><span class="chahao">×</span></div>');
...
$("#about").show();
$("#about").html("<span id=\"andmsg\">This page is optimized for iPhone</span>");
setTimeout('$("#about").fadeOut(1500)', 300);
$("#editDone").html('Edit');

setTimeout('window.scrollTo(0, 1)', 1000);
//setTimeout('alert(window.screen.availHeight)', 5000); setTimeout('alert(window.screen.availWidth)', 5000);
